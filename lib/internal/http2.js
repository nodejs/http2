...
class Http2Stream extends Duplex {
  ...
  setLocalWindowSize(size) {
    var handle = this[kHandle];
    if (handle) {
      handle.setLocalWindowSize(size);
    } else {
  ...
  changeStreamPriority(parentId, priority, exclusive) {
    var handle = this[kHandle];
    if (handle) {
      handle.changeStreamPriority(parentId, priority, exclusive);
    } else {
      this.once('handle',
  ...
  sendPriority(parentId, priority, exclusive) {
    var handle = this[kHandle];
    if (handle) {
      handle.sendPriority(parentId, priority, exclusive);
    } else {
      this.once('handle',
  ...
  sendPushPromise(headers) {
    ...
    var handle = this[kHandle];
    if (handle) {
      return handle.sendPushPromise(mapToHeaders(headers));
    } else {
  ...
  addTrailer(name, value, noindex) {
    var handle = this[kHandle];
    if (handle) {
      handle.addTrailer(name, value, noindex);
    } else {
...
class Http2Session extends EventEmitter {
  ...
  getRemoteSettings() {
    ...
    var handle = this[kHandle];
    if (handle)
      handle.getRemoteSettings(holder);
    return holder;
  }
  ...
  getLocalSettings() {
    ...
    var handle = this[kHandle];
    if (handle)
      handle.getLocalSettings(holder);
    return holder;
  }
  ...
  setLocalSettings(settings) {
    ...
    var handle = this[kHandle];
    if (handle) {
      handle.setLocalSettings(settings);
    }
  }
...
class Http2Outgoing extends Writable {
  ...
  setHeader(name, value, noindex) {
    ...
    headers.push([name, value, Boolean(noindex)]);
    return this;
  }
...
// Converts an object into an array of header entries.
...
function socketDestroy(destroySocket, session) {
  return function(error) {
    session.removeAllListeners();
    this.removeAllListeners();
    freeSession(session[kHandle]);
    this.destroy = destroySocket;
    destroySocket.call(this, error);
...
function connectionListener(socket) {
  ...
  socket.destroy = socketDestroy(destroySocket, session);
  ...
  session.setLocalSettings(options.settings);
}
...
function getDefaultSettings() {
  var holder = {};
  http2.getDefaultSettings(holder);
  return holder;
}
