...
class Http2Incoming extends Readable {
  ...
  _read(n) {
    this[kStream][kHandle].readStart();
  }
}
...
class Http2ServerRequest extends Http2Incoming {
  ...
  get authority() {
    return this[kHeaders][constants.HTTP2_HEADER_AUTHORITY];
  }
...
class Http2ServerResponse extends Http2Outgoing {
  ...
  [kImplicitHeaders]() {
    ...
    if (this.sendDate)
      this[kHeaders].unshift(['date', utcDate()]);
    this[kHeaders].unshift([constants.HTTP2_HEADER_STATUS, this.statusCode]);
  }
}
...
class Http2PushResponse extends EventEmitter {
  constructor(response) {
    ...
    this[kHeaders][constants.HTTP2_HEADER_METHOD] = 'GET';
    this[kHeaders][constants.HTTP2_HEADER_AUTHORITY] =
      response.stream[kRequest].authority;
    this[kHeaders][constants.HTTP2_HEADER_SCHEME] =
  ...
  set path(val) {
    this[kHeaders][constants.HTTP2_HEADER_PATH] = String(val);
  }
  ...
  set method(val) {
    this[kHeaders][constants.HTTP2_HEADER_METHOD] = String(val);
  }
  ...
  set authority(val) {
    this[kHeaders][constants.HTTP2_HEADER_AUTHORITY] = String(val);
  }
  ...
  set scheme(val) {
    this[kHeaders][constants.HTTP2_HEADER_SCHEME] = String(val);
  }
  ...
  push(callback) {
    if (typeof callback !== 'function')
      throw new TypeError('callback must be a function');
    var parent = this[kResponse].stream;
    var ret = parent.sendPushPromise(this[kHeaders]);
    if (ret) {
      var id = ret.getId();
      var stream = new Http2Stream(parent.session, id, {});
      stream._handle = ret;
...
function clientSessionOnHeaderComplete(stream, flags, headers, category) {
  ...
  switch (category) {
    case constants.NGHTTP2_HCAT_RESPONSE:
      ...
      var response = new Http2ClientResponse(stream, headers, {});
...
class Http2ClientRequest extends Http2Outgoing {
  ...
  setHeader(name, value) {
    name = String(name).toLowerCase().trim();
    var existing = this[kHeaders][name];
    if (existing) {
  ...
  setTrailer(name, value) {
    ...
    name = String(name).toLowerCase().trim();
    var existing = this[kTrailers][name];
    if (existing) {
      if (Array.isArray(existing)) {
        existing.push(String(value));
      } else {
  ...
  [kBeginSend]() {
    if (!this[kHeadersSent]) {
      this[kHeadersSent] = true;
      var stream = this[kStream];
      this[kImplicitHeaders]();
      var _handle = stream.session.request(mapToHeaders(this[kHeaders]), true);
...
function addTrailers() {
  ...
  if (request[kTrailers]) {
    ...
    for (var key in request[kTrailers]) {
      var value = request[kTrailers][key];
      if (Array.isArray(value) && value.length > 0) {
...
class Http2ClientResponse extends Http2Incoming {
  ...
  get status() {
    return this[kHeaders][constants.HTTP2_HEADER_STATUS] | 0;
  }
}
