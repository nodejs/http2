...
function onStreamClose(id, code) {
  ...
  if (stream) {
    this[kOwner].emit('streamClose', stream, code);
...
class Http2Stream extends Duplex {
  ...
  respond(nobody, headers) {
    if (this[kHandle]) {
      this[kHandle].respond(nobody, headers);
    } else {
      this.once('handle', onHandleRespond(nobody, headers));
    }
  }
  ...
  _write(data, encoding, cb) {
    if (this[kHandle]) {
      ...
      const err = createWriteReq(req, this[kHandle], data, enc);
      if (err)
...
function onHandleRespond(nobody, headers) {
  return function(nobody, headers) {
...
class Http2Session extends EventEmitter {
  ...
  request(headers, nobody) {
    if (this[kHandle])
      return this[kHandle].request(headers, nobody);
  }
}
...
function onHttp2OutgoingPipe() {
  if (!this[kHeadersSent]) {
    const beginSend = this[kBeginSend];
    beginSend.call(this);
  }
}
...
class Http2Outgoing extends Writable {
  ...
  [kBeginSend]() {
    if (!this[kHeadersSent]) {
      const implicitHeaders = this[kImplicitHeaders];
      if (typeof implicitHeaders === 'function') {
        implicitHeaders.call(this);
      }
      this[kHeadersSent] = true;
      this.stream.respond(Boolean(this[kNoBody]), this[kHeaders]);
    }
  }
}
...
class Http2ClientRequest extends Http2Outgoing {
  ...
  [kBeginSend]() {
    if (!this[kHeadersSent]) {
      ...
      const _handle = stream.session.request(mapToHeaders(this[kHeaders]), true);
