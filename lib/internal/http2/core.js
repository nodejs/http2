...
function onSessionStreamClose(id, code) {
  ...
  if (alreadyClosed)
    return;
  _unrefActive(this); // Reset the session timout timer
...
class Http2Session extends EventEmitter {
  ...
  settings(settings) {
    ...
    emitErrorIfNecessary(this, this._handle.submitSettings(settings));
  }
...
function streamOnResume() {
  ...
  if (session && !state.reading) {
    state.reading = true;
    emitErrorIfNecessary(this, session._handle.streamReadStart(this.id));
  }
}
...
function streamOnPause() {
  ...
  if (session && state.reading) {
    state.reading = false;
    emitErrorIfNecessary(this, session._handle.streamReadStop(this.id));
  }
}
