...
class Http2Session extends EventEmitter {
  constructor(type, options, socket, settings) {
    ...
    if (socket.connecting) {
      ...
      this.once('socketConnected', setupFn);
    } else {
      setupFn();
    }
...
function socketRenegotiate(options, callback) {
  if (typeof options === 'function') {
    callback = options;
    options = {};
  }
  if (typeof callback !== 'function')
    throw new errors.TypeError('ERR_INVALID_CALLBACK');
  const err =
    new errors.TypeError('ERR_HTTP2_SOCKET_TLS_RENEGOTIATION_FORBIDDEN');
  callback(err);
}
...
function connectionListener(socket) {
  ...
  if (typeof socket.renegotiate === 'function') {
    debug('replacing socket renegotiate method');
    socket.renegotiate = socketRenegotiate;
  }
...
function createClientSession(options, socket) {
  ...
  // 2. Disable TLS renegotiation on the socket
  if (typeof socket.renegotiate === 'function') {
    debug('replacing client socket renegotiate method');
    socket.renegotiate = socketRenegotiate;
  }
...
function connect(authority, options, listener) {
  ...
  switch (protocol) {
    case 'http:':
      socket = net.connect(port, host, socketConnected);
      break;
    case 'https:':
