...
class Http2Session extends EventEmitter {
  ...
  settings(settings) {
    ...
    switch (ret) {
      case NGHTTP2_ERR_INVALID_ARGUMENT:
        throw new errors.Errors('ERR_HTTP2_INVALID_SETTING_VALUE', settings);
      case NGHTTP2_ERR_NOMEM:
        throw new errors.Errors('ERR_OUTOFMEMORY');
      default:
        // Some other unexpected error was reported.
        if (ret < 0)
          throw new NghttpError(ret);
    }
  }
  ...
  priority(stream, options) {
    ...
    assert(stream.id,
           `The Stream ID is ${stream.id}. Please report this as a bug in ` +
           'Node.js');
    ...
    switch (ret) {
      case NGHTTP2_ERR_NOMEM:
        throw new errors.Error('ERR_OUTOFMEMORY');
      default:
  ...
  rstStream(stream, code) {
    ...
    const ret = handle.submitRstStream(stream.id, code);
    switch (ret) {
      case NGHTTP2_ERR_NOMEM:
        throw new errors.Error('ERR_OUTOFMEMORY');
      default:
...
function onHandleFinish() {
  ...
  if (this.id === undefined) {
    this.on('connect', onHandleFinish.bind(this));
  } else {
...
function streamOnResume() {
  if (this._paused)
    return this.pause();
  if (this.id === undefined) {
    this.on('connect', streamOnResume.bind(this));
    return;
  }
  const session = this.session;
  const state = this._state;
...
class ServerHttp2Stream extends Http2Stream {
  ...
  pushStream(headers, options, callback) {
    ...
    switch (ret) {
      case NGHTTP2_ERR_NOMEM:
        throw errors.Error('ERR_OUTOFMEMORY');
      case NGHTTP2_ERR_STREAM_ID_NOT_AVAILABLE:
